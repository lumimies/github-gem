...
flags :private => 'Create private repository'
flags :remotename => 'Name for the local remote (default: origin)'
flags :reponame => 'Name for the repository (default: $CWD)'
command :'create-from-local' do
  cwd = sh "pwd"
  remote = options['remotename'] || 'origin'
  repo = options['reponame'] || File.basename(cwd)
  ...
  if created.out =~ %r{You are being <a href="https://github.com/#{github_user}/([^"]+)"}
    git "remote add #{remote} git@github.com:#{github_user}/#{$1}.git"
    git_exec "push #{remote} master"
  else
